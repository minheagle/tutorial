...
1. **Preparation**: Prior to launching the containers, ensure the folders for the <code>export</code>, and <code>consume</code> volumes exist to prevent permission issues.

2. **Environment Variables**: Define necessary environment variables in a <code>.env</code> file, including <code>PG_PASS</code>, <code>PG_USER</code>, <code>PG_DB</code>, and <code>AUTHENTIK_SECRET_KEY</code>.

3. **Launching Paperless-ngx**:

    * Pull the latest images with <code>docker compose pull</code>.
    * Start the services using <code>docker compose up -d</code>.
